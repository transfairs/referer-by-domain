function createChromeMock({ refererHeaders = {}, sessionRelations = {}, withSessionStorage = true } = {}) {
  const listeners = {};
...
describe('background.js', () => {
  ...
  describe('onBeforeRequest', () => {
    ...
    test('starts with no relations when session storage has nothing stored yet', () => {
      const chromeMock = createChromeMock();
      chromeMock.storage.session.get = jest.fn((key, callback) => callback({}));
      const listeners = loadBackground(chromeMock);
    ...
    test('tracks relations without persisting when storage.session is unavailable', () => {
      const chromeMock = createChromeMock({ withSessionStorage: false });
      const listeners = loadBackground(chromeMock);
